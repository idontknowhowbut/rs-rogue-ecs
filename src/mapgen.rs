...
fn simulate_step(map: &Vec<TileType>, width: i32, height: i32, use_r2: bool) -> Vec<TileType> {
    ...
    for y in 0..height {
        for x in 0..width {
            ...
            new_map[idx_map(x, y, width)] = if wall { TileType::Wall } else { TileType::Floor };
...
fn flood_fill(map: &Vec<TileType>, start_x: i32, start_y: i32, width: i32, height: i32) -> Vec<(i32, i32)> {
    use std::collections::VecDeque;
    ...
    while let Some((x, y)) = queue.pop_front() {
        ...
        for (nx, ny) in [(x-1,y),(x+1,y),(x,y-1),(x,y+1)] {
            ...
            if visited[idx_map(nx, ny, width)] { continue; }
            if matches!(map[idx_map(nx, ny, width)], TileType::Wall) { continue; }
...
pub fn remove_isolated_regions(map: &mut Vec<TileType>, width: i32, height: i32) {
    ...
    for y in 0..height {
        for x in 0..width {
            ...
            if visited[idx_map(x, y, width)] { continue; }            
            if matches!(map[idx_map(x, y, width)], TileType::Wall) { continue; }
    ...
    let largest = regions.iter().enumerate()
